...
// Agent B creates a signature for the challenge using their key
receive(message => {
...
// Agent A verifies the signature using Agent B's profile
receive(message => {
  if (message.messageType === 'AUTHENTICATION_RESPONSE') {
    // Resolve the Agent ID to get the Agent Profile
    const profile = await resolveAgent(agentBId);

    // Verify the signature using the public key from the Agent Profile
    const isValid = await verifySignature(message.challenge, message.signature, profile);

    if (isValid) {
      // Authentication successful
      establishSecureChannel(agentBId);
    }
  }
});
```
