...
```javascript
// DID-based Challenge-Response Example
...
// Agent B creates a proof for the challenge using their verification method
receive(message => {
...
// Agent A verifies the proof using the DID Document
receive(message => {
  if (message.messageType === 'AUTHENTICATION_RESPONSE') {
    // Resolve the DID to get the DID Document
    const didDocument = await resolveDid(agentBDid);

    // Verify the proof using the verification method from the DID Document
    const isValid = await verifyProof(message.challenge, message.proof, didDocument);

    if (isValid) {
      // Authentication successful
      establishSecureChannel(agentBDid);
    }
  }
});
```
