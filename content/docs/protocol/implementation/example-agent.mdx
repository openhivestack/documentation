...
export const config = {
  agent: {
    id: process.env.AGENT_ID || 'advanced-hive-agent',
...
```typescript
// src/utils/crypto.ts
import { KeyPair } from '@hive-protocol/sdk';
...
export async function getKeyPair(): Promise<KeyPair> {
  // If key paths are provided, try to load existing keys
  if (config.keys.privateKeyPath && config.keys.publicKeyPath) {
    try {
      const privateKeyPem = await fs.readFile(config.keys.privateKeyPath, 'utf-8');
      const publicKeyPem = await fs.readFile(config.keys.publicKeyPath, 'utf-8');

      return KeyPair.fromPem(privateKeyPem, publicKeyPem);
    } catch (error) {
      console.warn('Failed to load existing keys, generating new ones', error);
    }
  }
  ...
  if (config.keys.privateKeyPath && config.keys.publicKeyPath) {
    try {
      ...
      await fs.mkdir(path.dirname(config.keys.privateKeyPath), { recursive: true });
      await fs.mkdir(path.dirname(config.keys.publicKeyPath), { recursive: true });

      // Save keys
      await fs.writeFile(config.keys.privateKeyPath, keyPair.privateKeyPem);
      await fs.writeFile(config.keys.publicKeyPath, keyPair.publicKeyPem);

      console.log('Generated and saved new key pair');
    } catch (error) {
      console.error('Failed to save generated keys', error);
    }
  }
...
export async function verifySignature(
  ...
  try {
    ...
    return KeyPair.verify(dataStr, signature, publicKey);
  } catch (error) {
...
import { TaskContract, KeyPair } from '@hive-protocol/sdk';
...
```typescript
// src/agent.ts
import { HiveAgent, KeyPair, ErrorCode } from '@hive-protocol/sdk';
import express from 'express';
import cors from 'cors';
import winston from 'winston';

import { config } from './config';
import { getKeyPair, validateParameters } from './utils';
...
async function main() {
  try {
    // Load or generate key pair
    logger.info('Initializing agent key pair');
    const keyPair = await getKeyPair();
    logger.info(`Agent public key: ${keyPair.publicKeyPem.substring(0, 32)}...`);

    // Create contract manager
    const contractManager = new ContractManager(keyPair);
