...
export const config = {
  agent: {
    ...
    didMethod: process.env.DID_METHOD || 'hive'
  },
  server: {
  ...
  did: {
    documentPath: process.env.DID_DOCUMENT_PATH,
    keyPath: process.env.DID_KEY_PATH
  },
  discovery: {
...
```typescript
// src/utils/crypto.ts
import { DidManager } from '@hive-protocol/sdk';
import fs from 'fs/promises';
import path from 'path';
import { config } from '../config';

// Load or generate DID and key pair
export async function getAgentDid() {
  const didManager = new DidManager();

  // If DID document path is provided, try to load existing DID
  if (config.did.documentPath && config.did.keyPath) {
    try {
      const didDocumentJson = await fs.readFile(config.did.documentPath, 'utf-8');
      const didKeyJson = await fs.readFile(config.did.keyPath, 'utf-8');

      const didDocument = JSON.parse(didDocumentJson);
      const keyPair = JSON.parse(didKeyJson);

      return { did: didDocument.id, didDocument, keyPair };
    } catch (error) {
      console.warn('Failed to load existing DID, generating new one', error);
    }
  }

  // Generate new DID
  const { did, didDocument, keyPair } = await didManager.generate({
    method: config.agent.didMethod,
    keyType: 'Ed25519'
  });
