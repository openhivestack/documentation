...
This quick start guide will help you build your first H.I.V.E. Protocol agent using **Tier 1 (Minimal)** implementation. You'll have a working agent that can process tasks and communicate with other agents.

## Why This Approach?

- **🚀 Fast**: Quick agent implementation
- **🔧 Simple**: Only 7 message types, minimal complexity
- **📈 Scalable**: Designed for concurrent agent operations
- **🎯 Focused**: Covers most use cases with minimal complexity
...
# Install minimal dependencies (Tier 1 only)
...
<Callout type="info">
  **Tier 1 Approach**: We're using minimal dependencies. No complex SDK required
  - just HTTP and crypto for signatures.
</Callout>
...
## Step 3: Add Message Handling (10 minutes)
...
function signMessage(message: any): string {
  ...
  return crypto
    .sign("sha256", Buffer.from(messageStr), privateKey)
    .toString("base64");
}
...
## Step 4: Add Discovery & Status Endpoints (5 minutes)
...
// Start the server
app.listen(PORT, () => {
  console.log(`🚀 H.I.V.E. Agent running at http://localhost:${PORT}`);
  console.log(`📋 Capabilities: ${capabilities.map((c) => c.id).join(", ")}`);
  console.log(`🔍 Try: curl http://localhost:${PORT}/status`);
});
```

## Step 5: Create a Simple Test Client (5 minutes)
...
function signMessage(message: any): string {
  const messageStr = JSON.stringify(message);
  return crypto
    .sign("sha256", Buffer.from(messageStr), clientPrivateKey)
    .toString("base64");
}
...
async function sendTaskRequest() {
  const taskRequest = {
    from: CLIENT_ID,
    to: "agentid:hive:agent", // Will be ignored by our simple agent
    type: "task_request",
    data: {
  ...
  try {
    ...
    console.log("📤 Sent:", taskRequest.data.text);
...
// Test the agent
console.log(`🧪 Testing agent with client: ${CLIENT_ID}`);
sendTaskRequest();
```

## Step 6: Set Up Scripts & Test (3 minutes)
...
```
Agent ID: agentid:hive:a1b2c3d4
🚀 H.I.V.E. Agent running at http://localhost:3000
...
```
🧪 Testing agent with client: agentid:hive:client-x1y2
✅ Task completed!
📤 Sent: Hello H.I.V.E. Protocol!
📥 Result: HELLO H.I.V.E. PROTOCOL!
```
...
- ✅ Handle the 7 core message types
- ✅ Advertise its capabilities
- ✅ Communicate with other agents

## What You Built (Tier 1 Features)
...
**Tier 1 provides most benefits with minimal complexity**:

- ✅ **Fast to implement**: Quick setup using familiar technologies
- ✅ **Easy to debug**: Simple message format
- ✅ **Highly scalable**: Designed for concurrent operations
- ✅ **Production ready**: Add security and monitoring
- ✅ **Upgrade path**: Clear path to more features

<Callout type="info">
  **Production Tip**: Many successful H.I.V.E. deployments start with Tier 1 and
  only upgrade specific components as needed. You don't need all features to be
  effective!
</Callout>
