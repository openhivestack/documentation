---
title: Testing
description: Simple testing for ultra-simple protocols
icon: TestTube
---
...
const TEST_CLIENT_ID = "agentid:hive:test-client";
...
async function testMessageFormat() {
  ...
  console.log(
    isValid ? "✅ Message format valid" : "❌ Message format invalid"
  );
  return isValid;
}
...
async function testCapabilities() {
  ...
  for (const test of tests) {
    ...
    console.log(
      success ? `✅ ${test.capability} works` : `❌ ${test.capability} failed`
    );
  }
}
...
async function testErrorHandling() {
  ...
  console.log(
    isErrorResponse ? "✅ Error handling works" : "❌ Error handling failed"
  );
}
...
async function testDiscovery() {
  ...
  console.log(hasCapabilities ? "✅ Discovery works" : "❌ Discovery failed");
}

// Run all tests
async function runTests() {
  console.log("🚀 H.I.V.E. Agent Test Suite\\n");
