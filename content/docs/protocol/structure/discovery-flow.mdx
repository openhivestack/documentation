...
async function findAndUseAgent(capability, taskData) {
  ...
  const response = await fetch(
    `https://registry.example.com/agents?capability=${capability}`
  );
...
**Tier 1 Discovery Requirements**:

- [ ] Generate unique Agent ID with `agentid:hive:` prefix
- [ ] Register with HTTP registry (optional)
- [ ] Query registry for capability discovery
- [ ] Implement basic agent selection
...
- [ ] Agent caching and offline handling
- [ ] Discovery result ranking
